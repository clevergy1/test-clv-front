...
$(function () {
    ...
    $(document).ready(function () {
        ...
        $('#ApplicationTitle').html('<span name="lbl" caption="edit">Add</span>&nbsp;<span name="lbl" caption="project">project</span>');
        $("#pageOperation").empty();
        ...
        function ReadProject() {
            ...
            req.success(function (json) {
                console.log("ReadProject", json);
                if (json.retval === true) {
                    var data = json.data[0];
                    $("#DesImpianto").val(data.description);
                    $("#Indirizzo").val(data.address.street);
                    $('#latitude').val(data.address.latitude);
                    $('#longitude').val(data.address.longitude);
                    $('#AltSLM').val(data.address.altitude);
        ...
        $('#Update').on('click', function (e) {
            ...
            if (checkUpd(DesImpianto) == false) {
                var req = $.DataAccess.projects_Update(localStorage.getItem("clv-projectId"),
                                                       $('#DesImpianto').val(),
                                                       $('#Indirizzo').val(),
                                                       Latitude,
                                                       Longitude,
