...
(function($) {
	...
	$.fn.zoomPanTouchSVG = function( options ) {
		...
		var settings = $.extend({		
			...
			initialMinY: 		false, 
			zoomInText: 		'<img src="images/ZoomIn.png" style="height:24px;width:24px" alt="" />',
			zoomOutText:        '<img src="images/ZoomOut.png" style="height:24px;width:24px" alt="" />',
		...
		if(!settings.zoomInBtnSrc) {
		    zoomInBtnHTML = '<button class="tZPSVGBtn btn btn-default" id="' + zoomInBtnID + '">' + settings.zoomInText + '</button>';
		} else {
		...
		if(!settings.zoomOutBtnSrc) {
		    zoomOutBtnHTML = '<button class="tZPSVGBtn btn btn-default" id="' + zoomOutBtnID + '">' + settings.zoomOutText + '</button>';
		} else {
		 ...
		 function zoom(direction) {
		 	if(direction === 'in' && curZoomIndex === (zoomLevels.length - 1)) {
				...
				return false;
		 		...
		 		//console.log('zoom curZoomIndex= ' + curZoomIndex);
	    ...
	    /**/
		$( document.getElementById($this.attr("id"))).bind('mousewheel', function (event) {
